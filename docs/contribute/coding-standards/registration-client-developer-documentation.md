...
| :---: | :--- |
| **Main Service class and method:** | PacketSyncServiceImpl.java - packetSync\(List packetsToBeSynced\) |
...
| :---: | :--- |
| **Main Service class and method:** | PacketExportService.getSyncedRecords\(\) - to fetch the packet to be exported. updateRegistrationStatus\(List exportedPackets\) - update the status once exported. |
...
| **Main Service class and methods** | JobConfigurationServiceImpl.executeAllJobs\(\) - This would load all the active jobs [List of Jobs](registration-client-developer-documentation.md#List-of-Jobs-) from the local db and trigger the jobs. |
| **Input Parameter:** | - |
...
| 22. | reg\_center\_type | It contains list of center types with respect to language code | Sync from server master table |
...
messages\_en.properties - Messages are in English language.  
